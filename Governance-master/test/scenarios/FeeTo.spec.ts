...
import swapswapV1Factory from '@swapswap/v1-core/build/swapswapV1Factory.json'
import swapswapV1Pair from '@swapswap/v1-core/build/swapswapV1Pair.json'
...
describe('scenario:FeeTo', () => {
  ...
  beforeEach('deploy swapswap v1', async () => {
    factory = await deployContract(wallet, swapswapV1Factory, [wallet.address])
  })
  ...
  describe('tokens', () => {
    ...
    beforeEach('make test tokens', async () => {
      const { timestamp: now } = await provider.getBlock('latest')
      const token0 = await deployContract(wallet, swap, [wallet.address, constants.AddressZero, now + 60 * 60])
      tokens.push(token0)
      const token1 = await deployContract(wallet, swap, [wallet.address, constants.AddressZero, now + 60 * 60])
      tokens.push(token1)
    })
    ...
    beforeEach('create fee liquidity', async () => {
      ...
      pair = new Contract(pairAddress, swapswapV1Pair.abi).connect(wallet)
