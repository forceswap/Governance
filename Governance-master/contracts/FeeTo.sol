...
contract FeeTo {
    ...
    function renounce(address pair) public returns (uint value) {
        ...
        TokenAllowState storage token0AllowState = tokenAllowStates[IswapswapV1Pair(pair).token0()];
        TokenAllowState storage token1AllowState = tokenAllowStates[IswapswapV1Pair(pair).token1()];
        ...
        if (
            ...
            value = IswapswapV1Pair(pair).balanceOf(address(this));
            if (value > 0) {
                ...
                assert(IswapswapV1Pair(pair).transfer(pair, value));
                IswapswapV1Pair(pair).burn(pair);
            }
    ...
    function claim(address pair) public returns (uint value) {
        ...
        if (
            ...
            if (value > 0) {
                ...
                assert(IswapswapV1Pair(pair).transfer(feeRecipient, value));
            }
        }
    }
}

interface IswapswapV1Pair {
