...
export async function governanceFixture(
  ...
  const swap = await deployContract(wallet, swap, [wallet.address, timelockAddress, now + 60 * 60])
  ...
  const governorAlpha = await deployContract(wallet, GovernorAlpha, [timelock.address, swap.address])
  expect(governorAlpha.address).to.be.eq(governorAlphaAddress)

  return { swap, timelock, governorAlpha }
}
